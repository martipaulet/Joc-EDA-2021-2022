...
public:
  ...
  // If p is os type Path then
  // p.first.first = id of origin city
  // p.first.second= id of destination city
  // p.second = vector containing all positions of the path
  ...
  /**
   * Returns the ids of all units of a player.
   */
  vector<int> my_units(int pl);
...
private:
  ...
  inline bool unit_ok (int id) const {
    return id >= 0 and id < total_units();
  }
...
inline int State::total_units () const {
  return unit_.size();
}
...
inline vector<int> State::my_units (int pl) {
  if (pl >= 0 and pl < (int)pl_units_.size()) return pl_units_[pl];
  else {
    cerr << "warning: units requested for player " << pl << endl;
    return vector<int>();
  }
}
